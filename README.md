...
| **Production** | Separate kitchens (e.g. **Cake Kitchen**, **Snacks Kitchen** — editable in Settings). Staff type the name of what they made (e.g. *Rasmalai Cake*); new names are saved and offered next time. At day end they enter sold / wasted counts. **Unsold items carry forward automatically** to the next day as "left over". |
| **Attendance** | Staff clock in, take breaks and clock out from their phone; request leave. Owner / manager see who is working now, a monthly register (click any day to correct it) and approve leave as paid or unpaid. |
| **Staff** | A record for every person — job title, kitchen, joining date, ID, address, emergency contact, bank / UPI, weekly off — plus a documents folder (ID proof, photo, contract…). Staff can also be added **without an app login**. |
| **Salary** | Recommended salary from attendance (present, half day, paid leave, week off), which the owner can override. Record advances, salary payments, bonuses and deductions; see what is **still to pay**, including unpaid amounts from earlier months, and pay it with one click. Payments are added to Expenses automatically. |
